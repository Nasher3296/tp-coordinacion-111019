...
Adicionalmente, ambas goroutines pueden publicar mensajes sobre las queues de coordinación de EOF's (`coordOutQueues`). El canal AMQP subyacente no es seguro para uso concurrente, por lo que el acceso a esas colas también está protegido con un mutex dedicado (`sendMu`). Una alternativa sería usar un mutex por cola, lo que permitiría que dos goroutines publiquen en colas distintas de forma verdaderamente paralela. Sin embargo, dado que los mensajes de coordinación son los menos frecuentes del sistema (se intercambian únicamente al finalizar la ingesta de un cliente), la diferencia de performance es insignificante. A cambio, un mutex por cola aumenta la complejidad del código y dificulta razonar sobre qué recurso está tomado en cada momento. Prefiero pagar ese pequeño precio.
